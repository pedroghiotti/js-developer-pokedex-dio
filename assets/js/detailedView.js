...
detailedView.ScrollToId = (id) => {
    ...
    const pageElementWidth = detailedView.pokemonList.getBoundingClientRect().width; // Largura do elemento de detalhes.
    const idElementWidth = document.querySelector('.listedId').getBoundingClientRect().width; // Largura do elemento de ID.
...
document.getElementById('detailViewReturnButton').addEventListener('click', () => {
...
(() => {
    ...
    detailedView.idList.innerHTML = detailedView.idList.innerHTML.replace('!', idListStr);
})();